...
let url = 'mongodb://localhost:27017/bread';
MongoClient.connect(url, (err, db)=>{
  ...
  app.get('/', (req, res)=>{
    ...
    bread.find(filter).count((err, count)=>{
      if(err){
        console.error(err);
      }
      let total = count
      ...
      res.render('list', {title: "Mongo BREAD",rows: docs, pagination:{page: page, limit: limit, offset: offset, pages: pages, total: total, url: url}, query: req.query});
    })
  })
})
